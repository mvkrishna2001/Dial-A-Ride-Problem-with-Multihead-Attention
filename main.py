...
def main(args, prt):
    ...
    if args['is_train']:
        ...
        if args['load_path']:
            ...
            checkpoints = re.compile('\d+')
            str_dirs = ''
            for file in dir_list:
                str_dirs += str(file)+' '
            resume_step = max(list(map(int, checkpoints.findall(str_dirs))))
            print('Training Resumed at: ', resume_step)
            # prt.print_out('Training Resumed ...')
        else:
        ...
        for step in range(args['n_train']):
            ...
            if step%args['log_interval'] == 0:
                ...
                prt.print_out('Train Step: {} -- Time: {} -- Train reward: {} -- Value: {}'\
                      .format(step + resume_step,time.strftime("%H:%M:%S", time.gmtime(\
